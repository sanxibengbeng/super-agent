...
// Hoisted to module scope: these are static and must not be recreated on every
// render, otherwise react-markdown re-parses the markdown on each stream chunk.
...
const MARKDOWN_COMPONENTS: Components = {
  ...
  th: ({ children }) => (
    <th className="px-3 py-1.5 text-left font-semibold text-gray-300 border-b border-gray-700">
      {children}
    </th>
  ),
  td: ({ children }) => (
    <td className="px-3 py-1.5 text-gray-300 border-b border-gray-800">{children}</td>
  ),
}

export const TextContentBlock = memo(function TextContentBlock({ block }: TextContentBlockProps) {
  return (
    <div
      className="text-sm text-gray-100 leading-relaxed prose prose-invert prose-sm max-w-none"
      data-testid="text-content-block"
    >
